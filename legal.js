...
const copy = {
  ...
  privacy: {
    he: ['מדיניות פרטיות', `<p class="legal-note">המדיניות מתארת איזה מידע RAVO שומרת כדי להפעיל מרחב ביתי או משרדי משותף, ואיזה מידע אינה אוספת.</p><h2>המידע שנשמר</h2><p><strong>חשבון:</strong> כתובת דוא״ל, סיסמה שמנוהלת באופן מוצפן על ידי שירות האימות, שם תצוגה ותפקיד במרחב.</p><p><strong>תוכן המרחב:</strong> משימות, קניות, מזווה, ארוחות, חשבונות, אירועי יומן, התראות ותוכן משרדי שהמשתמשים בוחרים להוסיף.</p><p><strong>מידע טכני:</strong> פרטי מכשיר הנדרשים להתראות, ודיווחי תקלה טכניים שמנוקים מתוכן אישי ככל האפשר.</p><h2>למה המידע משמש</h2><p>רק להפעלת השירות: סנכרון בין חברי המרחב, אכיפת הרשאות, הצגת מידע והתראות. תוכן המרחב אינו משמש לפרסום או לאימון מודלים.</p><h2>מה איננו עושים</h2><p>איננו מוכרים מידע אישי, איננו מציגים פרסומות, איננו מבצעים פרסום ממוקד ואיננו משתמשים בעוגיות מעקב של צד שלישי באתר הציבורי.</p><h2>מי רואה מה</h2><p>הגישה מבוססת חשבון, חברות במרחב והרשאות. ההרשאות נאכפות במסד הנתונים ואין גישה בין מרחבים נפרדים.</p><h2>ספקי תשתית</h2><p>השירות משתמש בספקי תשתית לאימות, אחסון נתונים, סנכרון, שליחת דוא״ל, התראות וניטור תקלות. מידע עשוי להישמר מחוץ לישראל לצורך הפעלת השירות.</p><h2>שמירה, זכויות ומחיקה</h2><p>המידע נשמר כל עוד החשבון קיים ובהתאם לצורכי השירות. אפשר לעיין ולתקן פרטים במוצר ולמחוק חשבון מתוך ההגדרות. לבקשת עותק או שאלה כתבו אל <a href="mailto:support@ravoapp.app">support@ravoapp.app</a>.</p><h2>אבטחה</h2><p>התקשורת מוצפנת, סיסמאות אינן נשמרות אצל RAVO כטקסט ומפתחות בעלי הרשאות מוגברות אינם נשלחים למכשיר. אף שירות אינו חסין לחלוטין.</p>`],
    en: ['Privacy Policy', `<p class="legal-note">This policy explains the information RAVO keeps to operate a shared household or office workspace, and what it does not collect.</p><h2>Information kept</h2><p><strong>Account:</strong> email address, a password handled in encrypted form by the authentication provider, display name and workspace role.</p><p><strong>Workspace content:</strong> tasks, shopping, pantry, meals, bills, calendar events, notifications and office content users choose to add.</p><p><strong>Technical information:</strong> device details needed for notifications and technical error reports stripped of personal content where possible.</p><h2>Purpose</h2><p>Information is used to operate the service: syncing members, enforcing permissions, displaying information and sending notifications. Workspace content is not used for advertising or model training.</p><h2>What we don’t do</h2><p>We don’t sell personal information, display advertising, run targeted advertising or use third-party tracking cookies on this public website.</p><h2>Access</h2><p>Access depends on an account, workspace membership and permissions enforced in the database. Separate workspaces cannot access each other.</p><h2>Infrastructure providers</h2><p>RAVO uses providers for authentication, data storage, synchronisation, email, notifications and error monitoring. Information may be processed outside Israel to operate the service.</p><h2>Retention and your rights</h2><p>Information is kept while the account exists and as needed to operate the service. Details can be viewed and corrected in the product, and accounts can be deleted in settings. For a copy or a question, email <a href="mailto:support@ravoapp.app">support@ravoapp.app</a>.</p><h2>Security</h2><p>Transport is encrypted, passwords are not stored by RAVO as plain text and privileged keys are not sent to devices. No service can promise absolute security.</p>`]
  },
  terms: {
